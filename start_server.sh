...
then
    ...
    then
	pkill -9 cvc
        ps axf | grep "node /home/menon18/270sim/js_cvc_interface_prod.js" | grep -v grep | awk '{print "kill -9 " $1}' | sh
        echo "Starting main server at $time"
        node ~/270sim/js_cvc_interface_prod.js 2>&1 > ~/270sim/logging/main_"$time" &
	...
	printf "alias nanomainlog=\"nano ~/270sim/logging/main_$time\"\nalias tailmainlog=\"tail -f ~/270sim/logging/main_$time\"\n" > sourcelogs
         ...
         then
   	    pkill -9 cvc
	    ps axf | grep "node /home/menon18/270sim/js_cvc_interface.js" | grep -v grep | awk '{print "kill -9 " $1}' | sh
            echo "Starting dev server at $time"
            node ~/270sim/js_cvc_interface.js 2>&1 > ~/270sim/logging/dev_"$time" &
